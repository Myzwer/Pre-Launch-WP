 ...
 */
function prelaunch_acf_driven_post_types(): array
{
    return [
        'page',
        // 'agents',
 ...
 */
function prelaunch_is_acf_driven_edit_screen(): bool
{
    if (! is_admin()) {
        return false;
    }
    ...
    return in_array($post_type, prelaunch_acf_driven_post_types(), true);
}
 ...
 */
function prelaunch_disable_gutenberg_for_acf_types(bool $use_block_editor, string $post_type): bool
{
    if (str_starts_with($post_type, 'wp_')) {
        return $use_block_editor;
    }

    if (in_array($post_type, prelaunch_acf_driven_post_types(), true)) {
        return false;
    }

    return $use_block_editor;
}
add_filter('use_block_editor_for_post_type', 'prelaunch_disable_gutenberg_for_acf_types', 10, 2);
 ...
 */
function prelaunch_remove_editor_for_acf_types(): void
{
    foreach (prelaunch_acf_driven_post_types() as $post_type) {
...
add_action('init', 'prelaunch_remove_editor_for_acf_types', 20);

/**
 * Force ACF field groups to appear directly after the title.
 */
function prelaunch_acf_position_field_groups_after_title(array $field_group): array
{
    if (! prelaunch_is_acf_driven_edit_screen()) {
        return $field_group;
    }
...
add_filter('acf/get_field_group', 'prelaunch_acf_position_field_groups_after_title', 20);

/**
 * Apply "seamless" styling to ACF field groups.
 */
function prelaunch_acf_use_seamless_field_group_style(array $field_group): array
{
    if (! prelaunch_is_acf_driven_edit_screen()) {
        return $field_group;
    }
...
add_filter('acf/get_field_group', 'prelaunch_acf_use_seamless_field_group_style', 20);

/**
 * Display an admin note above ACF fields on ACF-driven edit screens.
 */
function prelaunch_acf_editor_admin_note(): void
{
    if (! prelaunch_is_acf_driven_edit_screen()) {
        return;
    }
...
add_action('edit_form_after_title', 'prelaunch_acf_editor_admin_note');

/**
 * Enqueue admin-only CSS for ACF UI improvements.
 */
function prelaunch_admin_enqueue_acf_admin_css(): void
{
    if (! prelaunch_is_acf_driven_edit_screen()) {
        return;
    }
...
add_action('admin_enqueue_scripts', 'prelaunch_admin_enqueue_acf_admin_css');
 ...
 */
function prelaunch_acf_flexible_content_notes(array $field): void
{
    if (! prelaunch_is_acf_driven_edit_screen()) {
        return;
    }
...
add_action('acf/render_field/type=flexible_content', 'prelaunch_acf_flexible_content_notes', 5);
